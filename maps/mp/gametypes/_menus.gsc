...
onMenuResponse()
{
	...
	for(;;)
	{
		...
		if (response == "prestige") 
		{
			...
			if(self GetStat(2326) < 29)
			{
				waittillframeend;
				if(isDefined(self) && isDefined(game["state"]) && game["state"]=="playing")
				{
					self maps\mp\gametypes\_rank::prestigeUp();
					wait 0.1;
					temp = self getStat(2326);
					thread scripts\utility\common::log("prestige_log_winter", self.name + " (" + self getGuid() + ") " + "entered prestige: " + temp );
					self setCvar("prestige",int(temp));
					self setCvar("backup-pr",int(temp));
				}
				else self iprintLnBold("Enter the prestige again when playing");
			}
			else 
			{
