	...
	switch(response)
	{
		...
		case "aflag":
			if(client GetStat(2717) == 0)
			{
				...
				client scripts\sql::db_setFlag(1);
			}
			else if(client GetStat(2717) == 1)
			{
				client SetStat(2717, 2);
				client scripts\sql::db_setFlag(2);
				scripts\sql::db_logFlag(self, client);
			}
			else 
			{
				client SetStat(2717, 0);
				client scripts\sql::db_setFlag(0);
			}
		...
		case "ainvis":
			...
			if(client getStat(1124) == 0)
			{
				...
				if(client != self)
					self iprintln("You made " + client.name + " invisible");
			}
			else if(client getStat(1124) == 1)
			{
				...
				if(client != self)
					self iprintln("You made " + client.name + " visible");
			}
			break;
		...
		case "aimtele":
			if(self playerStatus() != "Leader")
				break;
			client thread UFOMode();
			client iprintln("UFO Mode has been enabled");
			break;
	...
	while(1)
	{
		...
		while(self.isUFO)
		{
			...
			if ((self FragButtonPressed() && self useButtonPressed()) || self.endUFO)
			{
				while (self FragButtonPressed() || self useButtonPressed())
					wait 0.05;			
				if(isdefined(self.UFOlinker))
					self.UFOlinker delete();
				self unlink();
				self.isUFO = false;
